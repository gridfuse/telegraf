...
func (n *natsConsumer) Init() error {
	...
	for i, p := range n.SubjectParsing {
		...
		if len(splitMeasurement) != len(n.SubjectParsing[i].splitSubject) && len(splitMeasurement) != 1 {
			n.Log.Errorf("config error subject parsing: measurement length %d does not equal subject length %d", len(splitMeasurement), len(n.SubjectParsing[i].splitSubject))
